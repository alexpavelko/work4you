...
from work4you_app.models import Vacancy, Company, Category, EmploymentType, Currency, CandidateType
...
def find_vacancies(request):
    ...
    employment_types = list(map(lambda type: EmploymentType.objects.get(title=type), request.GET.getlist('employment_types')))
    candidate_types = list(map(lambda candidate_type: CandidateType.objects.get(title=candidate_type), request.GET.getlist('candidate_types')))
