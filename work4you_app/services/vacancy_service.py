...
from work4you_app.models import Category, Vacancy, EmploymentType, CandidateType, Currency, City
...
def get_vacancyForm_data(vacancy, vacancy_form):
    ...
    vacancy.currency = vacancy_form.cleaned_data['currency']
...
def find_vacancies(request):
    ...
    vacancies = Vacancy.objects.filter(
        categories__in=categories, salary__gte=salary, currency=currency, employment_types__in=employment_types,
        candidate_types__in=candidate_types, company__city__in=cities, title__contains=query
    ).distinct()
    context = {
